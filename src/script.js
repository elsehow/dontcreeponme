...
my.socket.on('message', function(data) {
  ...
  var ciphertext = JSONB.parse(data['message'])
  var buff = box.multibox_open(ciphertext, my.keypair.secretKey, Object.keys(my.current_userlist).length)
  var dec = buff.toString()

	var from = data['pseudo'];
	addMessage(dec, from, false);
...
function updateUserlistDiv() { 
  ...
  // update publickeys list
  my.pks = $.map(my.current_userlist, function (userObj) {
    return new Buffer(userObj.publicKey)
  })
	...
	$.each(my.current_userlist,function(username,userObj)  {

		if (username) {
			count++;
			userlistDiv.append(
				$('<span>' + username + ' </span>').css('color','#'+userObj.color)
			);
		}
...
function createMessageDiv(msg, pseudo, date, fromSelf, isAnnouncement) {
	...
	if (!isAnnouncement) {
		...
		else $div.find(".msgcolor").css('background-color','#'+my.current_userlist[pseudo].color);
	}
...
function setPseudo() {
	if ($("#pseudoInput").val() != "")
	{
        ...
        publicKey: my.keypair.publicKey,
